...
def test_erc20_transfer(royalty_token, WhiteList):
    tx = royalty_token.getWhiteList({'from': accounts[0]})
    white_list = WhiteList.at(tx.return_value)
    tx = white_list.add([accounts[2]],{'from': accounts[0]})
...
def test_erc20_transfer_from(royalty_token, WhiteList):
    tx = royalty_token.getWhiteList({'from': accounts[0]})
    white_list = WhiteList.at(tx.return_value)
    tx = white_list.add([accounts[3]],{'from': accounts[0]})
    assert 'AccountListed' in tx.events
...
def test_btts_disableMinting(royalty_token, WhiteList):
    tx = royalty_token.setMinter(accounts[2],{'from': accounts[0]})
    assert 'MinterUpdated' in tx.events
    tx = royalty_token.getWhiteList({'from': accounts[0]})
    white_list = WhiteList.at(tx.return_value)
